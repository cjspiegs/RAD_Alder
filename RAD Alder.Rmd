...
merged.true_home_z.df <- merged.true_home_z.df %>% 
  mutate(treatment = factor(treatment, levels = c("AB", "P")))%>%
           mutate(group = factor(group, levels =c("AW", "H")))
...
merged.true_home_z.df <- merged.true_home_z.df %>% 
  mutate(treatment = factor(treatment, levels = c("P", "AB")))%>%
           mutate(group = factor(group, levels =c("H", "AW")))
#Day 1
...
d1_meta.plot=merged.true_home_z.df%>%
  ...
  coord_cartesian(ylim = c(0,4.5))+
      scale_x_discrete(limits = c("P-H", "P-AW", "AB-H", "AB-AW")) +
  xlab("")+
  ylab(expression("optical density (OD"[450~nm]*")"))+
  labs(fill = "hfa")+
  ggtitle("Day 1")+
...
merged.true_home_z.df %>%
  na.omit() %>%
  filter(pairing == "P-H") %>%
  ggplot(aes(x = as.factor(decomp_day), y = od_corrected_b_n, fill = pairing)) +
  stat_summary(fun = "median", geom = "point", shape = 18, size = 4, color = "black") +
  stat_summary(fun = "median", geom = "point", shape = 18, size = 4, color = "black") +
  scale_fill_colorblind() +
  coord_cartesian(ylim = c(0, 5)) +
  geom_line(stat = "summary", fun = "median", aes(group = decomp_day), color = "black", size = 1) +
  theme_classic()

paw.boxplot=merged.true_home_z.df%>%
  na.omit()%>%
  filter(pairing == "P-AW")%>%
    ggplot(aes(x= pairing, y = od_corrected_b_n, fill = pairing))+
  geom_boxplot(alpha = 0.3)+
    scale_fill_colorblind()+
  coord_cartesian(ylim= c(0,5))+
  facet_wrap(decomp_day~. , nrow=1)+
  theme_classic()

abh.boxplot=merged.true_home_z.df%>%
  na.omit()%>%
  filter(pairing == "AB-H")%>%
    ggplot(aes(x= pairing, y = od_corrected_b_n, fill = pairing))+
  geom_boxplot(alpha = 0.3)+
...
abaw.boxplot=merged.true_home_z.df%>%
  na.omit()%>%
  filter(pairing == "AB-AW")%>%
    ggplot(aes(x= pairing, y = od_corrected_b_n, fill = pairing))+
  geom_boxplot(alpha = 0.3)+
    scale_fill_colorblind()+
  coord_cartesian(ylim= c(0,5))+
...
plot_grid(
  ...
  abh.boxplot,
  abaw.boxplot,
  rel_widths = c(8,8,8,8)
)


ggsave("figures/all.packs.plot.pdf", all.packs.plot, width = 22, height = 10)
```



12/14/23: model exploration for gams and boxplots (anova); final figs
```{r}
#### using gams
view(merged.true_home_z.df)

true_home.assay.plot
merged.true_home_z.df%>%
  na.omit() %>%
  ggplot(aes(x = as.factor(decomp_day), y = z_score, color = interaction(treatment,group), group = interaction(treatment,group))) +
  stat_summary(fun = mean, geom = "point", show.legend = F, size = 2.3) +
  geom_smooth(aes(linetype= pairing, fill = pairing),method = 'gam', formula = y ~ s(x, k = 6), alpha = 0.15) +
...
# present gam models
p.true_home.gam=gam(z_score ~ s(decomp_day, k=6) + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="P",])
summary(p.true_home.gam)

p.true_home.gam2=gam(z_score ~ group + s(decomp_day, by= group, k = 6) + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="P",])

p.true_home.gam3=gam(z_score ~ group + s(decomp_day, by= group, k = 6, bs="ps") + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="P",])
summary(p.true_home.gam3)

p.true_home.gam4=gam(z_score ~ group + s(decomp_day, by= group, k = 6, bs="gp") + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="P",])

p.true_home.gam5=gam(z_score ~ group + s(decomp_day, k = 6, bs="fs") + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="P",])

AIC(p.true_home.gam,p.true_home.gam2,p.true_home.gam3,p.true_home.gam4, p.true_home.gam5)
...
#plot with model
p.tue_home.gam.plot=plot_smooths(
model = p.true_home.gam,
series = decomp_day,
) + 
stat_summary(data = na.omit(merged.true_home_z.df[merged.true_home_z.df$treatment == "P",]), 
             fun = mean, geom = "point", size = 1.8, aes(color = group, x = decomp_day)) +
ggtitle("Present treatment only") +
xlab("decomposition day") +
ylab(expression("standardized optical density (OD"[450~nm]*")")) +
scale_color_colorblind() +
scale_fill_colorblind() +
geom_hline(aes(yintercept = 0, linetype = "Average"), color = alpha("red", 1), linetype = "dotted") +
...
merged.true_home_z.df%>%
    na.omit() %>%
  filter(treatment == "P")%>%
  ggplot(aes(x = as.factor(decomp_day), y = z_score, color = group, group = group)) +
  stat_summary(fun = median, geom = "point", size = 2.3) +
  geom_smooth(aes(linetype= group, fill = group),method = 'gam', formula = y ~ s(x, k = 6), alpha = 0.15) +
    ggtitle("Present treatment only")+
  xlab("decomposition day")+
    ylab(expression("standardized optical density (OD"[450~nm]*")"))+
  scale_color_colorblind()+
...
# absent gam
ab.true_home.gam=gam(z_score ~ group + s(decomp_day, k = 6) + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="AB",])

ab.true_home.gam2=gam(z_score ~ group + s(decomp_day, by= group, k = 6) + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="AB",])
ab.true_home.gam3=gam(z_score ~ group + s(decomp_day, by= group, k = 6, bs="ps") + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="AB",])
ab.true_home.gam4=gam(z_score ~ group + s(decomp_day, by= group, k = 6, bs="gp") + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="AB",])
ab.true_home.gam5=gam(z_score ~ group + s(decomp_day, k = 6, bs="fs") + s(tree_id, bs= "re") + s(tree_num, bs= "re"), data = merged.true_home_z.df[merged.true_home_z.df$treatment=="AB",])


AIC(ab.true_home.gam3,ab.true_home.gam5)
summary(ab.true_home.gam2)
gam.check(ab.true_home.gam2)

#plot differences
plot_difference(
  ab.true_home.gam3,
  series = decomp_day,
  difference = list(group = c("AW", "H"))
)+
  Fig.formatting

summary(ab.true_home.gam3)
#plot with model
ab.tue_home.gam.plot=plot_smooths(
model = ab.true_home.gam3,
series = decomp_day,
  comparison = group
) + 
stat_summary(data = na.omit(merged.true_home_z.df[merged.true_home_z.df$treatment == "AB",]),
              fun = mean, geom = "point", size = 1.8, aes(group = group, color = group, x = decomp_day)) +
...
merged.true_home_z.df%>%
  ...
  filter(treatment == "AB")%>%
  ggplot(aes(x = as.factor(decomp_day), y = z_score, color = group, group = group)) +
  stat_summary(fun = mean, geom = "point", size = 2.3) +
  geom_smooth(aes(linetype= group, fill = group),method = 'gam', formula = y ~ s(x, k = 6), alpha = 0.15) +
  ggtitle("Absent treatment only")+
  xlab("decomposition day")+
    ylab(expression("standardized optical density (OD"[450~nm]*")"))+
  ...
  scale_fill_colorblind()+
      geom_hline(aes(yintercept = 0, linetype = "Average"), color = alpha("red", 1), linetype = "dotted") +
  coord_cartesian(ylim = c(-1,1))+
      scale_x_discrete(expand = c(0.03, 0.03)) +  # Adjust the expand argument
  theme(
    axis.text = element_text(size = 12),
    axis.title = element_text(size = 14),
    legend.text = element_text(size = 12),
...
#present gam and boxplot merged
present.meta.gam_box.plot=plot_grid(
 p.meta.true_home.boxplot,
   p.tue_home.gam.plot,
  rel_widths = c(8,8),
  ncol=2
...
#absent gam and boxplot merged
absent.meta.gam_box.plot=plot_grid(
 ab.meta.true_home.boxplot,
   ab.tue_home.gam.plot,
  rel_widths = c(8,8),
  ncol=2
...
#combined
p.ab.meta.gam_box.plot=
  plot_grid(
 present.meta.gam_box.plot,
   absent.meta.gam_box.plot,
  rel_widths = c(8,8),
  nrow=2
)

ggsave("figures/p.ab.meta.gam_box.plot.pdf", p.ab.meta.gam_box.plot, width = 24, height = 18)
...
tw_gam_trt.grp=gam(od_corrected_b_n ~ treatment * group + s(decomp_day, k=6),family = tw(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))
plot(tw_gam_trt.grp)
summary(tw_gam_trt.grp)
anova.gam(tw_gam_trt.grp)

tw_gam_trt.grp=gam(od_corrected_b_n ~ treatment * group + s(decomp_day, by= treatment, k=6)  + s(decomp_day, by= group, k =6)+ s( tree_num, tree_id, bs="re") + s(site_num,bs="re") + s(deploy, bs="re"),  subset = hours == "24",family = tw(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))

tw_gam_trt.grp2=gam(od_corrected_b_n ~ treatment + group + s(decomp_day, by= treatment, k=6)  + s(decomp_day, by= group, k =6)+ s( tree_num, tree_id, bs="re")  + s(deploy, bs="re"),  subset = hours == "24",family = tw(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))

tw_gam_trt.grp3=gam(od_corrected_b_n ~ treatment * group + s(decomp_day, k =6)+ s( tree_num, tree_id, bs="re") + s(site_num,bs="re") + s(deploy, bs="re"),  subset = hours == "24",family = tw(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))

tw_gam_trt.grp4=gam(od_corrected_b_n ~ treatment*group + s(decomp_day, treatment, bs="fs", k =6)+ s(decomp_day,group,bs="fs", k =6)+ s(tree_id, bs="re") + s(site_num, deploy, bs="re"),  subset = hours == "24",family = tw(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))
...
z_tw_gam_trt.grp=gam(z_score ~ treatment * group + s(decomp_day, by= treatment, k=6)  + s(decomp_day, by= group, k =6)+ s( tree_num, tree_id, bs="re") + s(site_num,bs="re") + s(deploy, bs="re"),  subset = hours == "24",family = tw(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))


z_gam_trt.grp=gam(z_score ~ treatment * group + s(decomp_day, by= treatment, k=6)  + s(decomp_day, by= group, k =6)+ s( tree_num, tree_id, bs="re") + s(site_num,bs="re") + s(deploy, bs="re"),  subset = hours == "24",family = gaussian(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))

gam_trt.grp=gam(od_corrected_b_n ~ treatment * group + s(decomp_day, by= treatment, k=6)  + s(decomp_day, by= group, k =6)+ s( tree_num, tree_id, bs="re") + s(site_num,bs="re") + s(deploy, bs="re"),  subset = hours == "24",family = gaussian(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))
summary(gam_trt.grp)
gam.check(gam_trt.grp)
concrvity(gam_trt.grp)
anova.gam(gam_trt.grp)



AIC(z_gam_trt.grp, gam_trt.grp)
...
gam_pair=gam(od_corrected_b_n ~ pairing + s(decomp_day, by= pairing, k=6) + s( tree_num, tree_id, bs="re") + s(site_num,bs="re") + s(deploy, bs="re"),  subset = hours == "24",family = tw(link = "identity"), method = "REML", data = na.omit(merged.true_home_z.df))
summary(gam_pair)
gam.check(gam_pair)
...
mass.df%>%
  filter(treatment == "P")%>%
  ggplot(aes(x=treatment, y = perc.mass_loss, color = hfa))+
geom_boxplot()
...
12/13/23 model and plot for z scored mass loss; final fig
```{r}
library(multcompView)
...
#treatment only
present.z_contrast.mass.plot=mass.deploy.merged.df%>%
  filter(treatment == "P")%>%
...
aov4=aov(z_score ~ treatment , data = na.omit(mass.deploy.merged.df))
...
z_contrast.mass.plot=mass.deploy.merged.df%>%
  ...
  Fig.formatting
z_contrast.mass.plot

####at home only
home_mass_aov1=aov(z_score ~ treatment, subset= hfa == "H", data = na.omit(mass.deploy.merged.df))
home_mass_tHSD=TukeyHSD(home_mass_aov1, ordered = F, conf.level = 0.95)
home_cld <- multcompLetters4(home_mass_aov1, home_mass_tHSD)

# table with factors and 3rd quantile
home_mass.table <- mass.deploy.merged.df%>%
  filter(hfa=="H")%>%
...
# extracting the compact letter display and adding to the Tk table
home_cld <- as.data.frame.list(home_cld$treatment)
home_mass.table$cld <- home_cld$Letters
...
mass.deploy.merged.df%>%
  ...
  xlab("")+
  ylab("% mass loss (z scored)")+
  coord_cartesian(ylim(0,65))+
    labs(shape = "site", fill = "treatment") +  # Change "Your New Legend Title" to your desired title
  scale_fill_colorblind()+
      geom_text(data = home_mass.table, aes(y = quant, label = cld), color = "blue", fontface= "bold",size = 5, vjust=-1.3, hjust =-0.76)+
  geom_hline(data = mass.deploy.merged.df, aes(yintercept = mean(z_score), linetype = "Average"), color = "red", linetype = "dotted", size= 1) +
  Fig.formatting
z_contrast.mass.plot
...
ggplot(aes(x = hfa, y = z_score)) +
  ...
  geom_hline(aes(yintercept = mean(z_score), linetype = "Average"), color = "red", linetype = "dotted") +
...
Generic t-test for H only treatment
```{r}
...
# Merge the dataframes for Armla and Brmla
ab.site_1.armla.brmla_df <- rbind(ab.site_1.armla_df, ab.site_1_brmla_df)
...
# Merge the dataframes for Armla and Brmla
ab.site_1.brmlb.armlb_df <- rbind(ab.site_1_brmlb_df, ab.site_1.armlb_df)
...
t_test <- t.test(positive_values[positive_values$treatment == "P",]$values, positive_values[positive_values$treatment == "AB",]$values)

hfai.allsites.df%>%
  filter(treatment == "AB", values > 0)%>%
n_distinct()
#41 Present HFA
#14 Absent HFA
...
```{r}
#load data
isotope.df=read.csv("/Users/Cody1/Desktop/GitHub/RAD Alder/data/isotopes.csv") 
...
isotope.df=isotope.df%>%
  group_by(treatment, tree_id, site_num, tree_num)%>%
    ...
    mutate(total_N.g=total_N/10^6)%>%
    mutate(c_n = total_C.g / total_N.g)%>%
    mutate(weight.g = weight/1000)%>%
    mutate(perc.C= (total_C.g/weight.g)*100)%>%
    mutate(perc.N= (total_N.g/weight.g)*100)
...
C_N.plot=isotope.df %>%
  ...
  scale_fill_colorblind()+
  geom_hline(aes(yintercept = mean(c_n), linetype = "Average"), color = alpha("red", 1), linetype = "dashed") +
  theme_classic()+
  theme(plot.title = element_text(face = "bold"))+
  geom_text(data = c_n.table, aes(y = quant, label = cld), color = "blue", fontface= "bold",size = 5, vjust=-2, hjust =-0.76)+
  theme_classic()
