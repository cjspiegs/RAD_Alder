...
12/13/23 updated: TRUE HOME (HOK to HOK or SEK to SEK)
...
merged.true_home_z.df%>%
  na.omit() %>%
  ggplot(aes(x = as.factor(decomp_day), y = z_score, color = treatment, group = treatment)) +
  stat_summary(fun = mean, geom = "point", show.legend = F, size = 2.3) +
  geom_smooth(aes(linetype= pairing, fill = pairing),method = 'gam', formula = y ~ s(x, k = 6), alpha = 0.15) +
...
mean(merged.true_home_z.df[merged.true_home_z.df$treatment=="P" & merged.true_home_z.df$decomp_day == "3",]$z_score)
...
aw.h.trt_boxplot=merged.true_home_z.df%>%
  ...
  geom_boxplot(alpha= 0.5)+
  xlab("group")+
    ggtitle("PSM metabolization between groups")+
    ylab(expression("standardized optical density (OD"[450~nm]*")"))+
  scale_fill_colorblind()+
    geom_hline(aes(yintercept = 0, linetype = "Average"), color = alpha("red", 1), linetype = "dashed") +
...
#treatment P vs AB only boxplots
day1.meta_aov1=aov(z_score ~ treatment, subset = decomp_day == "1", data = na.omit(merged.true_home_z.df))
day3.meta_aov1=aov(z_score ~ treatment, subset = decomp_day == "3", data = na.omit(merged.true_home_z.df))
day5.meta_aov1=aov(z_score ~ treatment, subset = decomp_day == "5", data = na.omit(merged.true_home_z.df))
day7.meta_aov1=aov(z_score ~ treatment, subset = decomp_day == "7", data = na.omit(merged.true_home_z.df))
day11.meta_aov1=aov(z_score ~ treatment, subset = decomp_day == "11", data = na.omit(merged.true_home_z.df))
day17.meta_aov1=aov(z_score ~ treatment, subset = decomp_day == "17", data = na.omit(merged.true_home_z.df))
...
ggsave("figures/meta_box.plot.pdf", meta_box.plot, width = 24, height = 18)
...
(-(log(mass.df$final_correct/mass.df$initial_mass)))
...
trt.mass_loss.plot=mass.deploy.merged.df%>%
